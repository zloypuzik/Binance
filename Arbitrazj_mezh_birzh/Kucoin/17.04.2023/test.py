...
class Birzi:
    ...
    def Binance():
        ...
        price_list = requests.get(
            "https://api.binance.com/api/v3/ticker/24hr").json()  # Получим ответ в виде JSON формата
    ...
    #     # Получим по ссылки данные с ценами.
    #     price_list = requests.get(
    #         "https://api.kucoin.com/api/v1/market/allTickers").json()  # Получим ответ в виде JSON формата
    #
    #     price = []
    #     # Переберём в цикле все торговые пары
...
def connectDB():
    # return MySQLdb.connect(host=Setting.setting["host"], user=Setting.setting["user"], passwd=Setting.setting["passwd"],
    #                        db=Setting.setting["db"])
    return pymysql.connect(host='192.168.1.50', port=3306, user='test', password='Zz_479621212', database='arbitrazh_mezbirzh')
...
def save_price(price: list, birza: int):
    ...
    #     else:
    #         changes = 0
    #print(birza, symbol, minut)
        # cursor.execute(
        #     f"INSERT INTO `price` (`birza`, `symbol`, `price`) VALUES ({birza}, '{symbol[0]}', '{symbol[1]}') ON DUPLICATE KEY UPDATE price = '{symbol[1]}' , `prevDay` = {changes} ,last_update = UNIX_TIMESTAMP();")  # Записать изменение цены
        #
